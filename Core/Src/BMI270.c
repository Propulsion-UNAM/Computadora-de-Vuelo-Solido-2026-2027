...
{
    uint8_t txData[3] = {0x80 | 0x00, 0xFF, 0xFF};
    uint8_t rxData[3] = {0, 0, 0};
    SPI_Recive(txData, rxData, 3, 100,bmi270);
    return rxData[2];
}
bool SPI_Transmit(uint8_t *pdata,uint8_t size,uint32_t timeout,BMI270_t *bmi270)
{
	bool res = false;
	HAL_GPIO_WritePin(bmi270->cs_port, bmi270->cs_pin, GPIO_PIN_RESET);
	res = (HAL_SPI_Transmit(bmi270->hspi, pdata, size, timeout)) != HAL_OK ? false:true;
	HAL_GPIO_WritePin(bmi270->cs_port, bmi270->cs_pin, GPIO_PIN_SET);
	return res;
}
bool SPI_Recive(uint8_t *txdata,uint8_t*rxdata,uint8_t size,uint32_t timeout,BMI270_t *bmi270)
{
	 bool res = false;
	 HAL_GPIO_WritePin(bmi270->cs_port,bmi270->cs_pin , GPIO_PIN_RESET);
	 res = (HAL_SPI_TransmitReceive(bmi270->hspi, txdata, rxdata, size, timeout)) != HAL_OK ? false : true;
	 HAL_GPIO_WritePin(bmi270->cs_port, bmi270->cs_pin, GPIO_PIN_SET);
	 return res;
}
...
void BMI270_gyro_calibrate(BMI270_t *bmi270)
{
	...
	//set gyro 3.2khz and normal mod filter
	uint8_t conf_tx[2] = {0x42, 0x2D};
	SPI_Transmit(conf_tx, 2, 100, bmi270);
	HAL_Delay(1);

	//2000 dps config
	uint8_t range_tx[2] = {0x43, 0x00};
	SPI_Transmit(range_tx, 2, 100, bmi270);
	HAL_Delay(1);

    // Configurar acelerómetro a rango de +-16g (Registro 0x41, valor 0x03)
    uint8_t acc_range_tx[2] = {0x41, 0x03};
    SPI_Transmit(acc_range_tx, 2, 100, bmi270);
    HAL_Delay(1);
	...
	int32_t sum_y = 0;
	int32_t sum_z = 0;
	uint8_t read_reg = 0x12 | 0x80;
	uint8_t raw_rx[7] = {0,0,0,0,0,0,0};
	...
	for(int i = 0; i < 2000; i++)
	{
		...
		HAL_GPIO_WritePin(bmi270->cs_port, bmi270->cs_pin, GPIO_PIN_RESET);
		HAL_SPI_Transmit(bmi270->hspi, &read_reg, 1, 100);
		HAL_SPI_Receive(bmi270->hspi, raw_rx, 7, 100);
		HAL_GPIO_WritePin(bmi270->cs_port, bmi270->cs_pin, GPIO_PIN_SET);

		sum_x += (int16_t)((raw_rx[2] << 8) | raw_rx[1]);
		sum_y += (int16_t)((raw_rx[4] << 8) | raw_rx[3]);
		sum_z += (int16_t)((raw_rx[6] << 8) | raw_rx[5]);

		HAL_Delay(1);

	}

	bmi270->offset_x = (float)sum_x / 2000.0f /16.4f;
	bmi270->offset_y = (float)sum_y / 2000.0f /16.4f;
	bmi270->offset_z = (float)sum_z / 2000.0f /16.4f;
...
uint8_t BMI270_init(BMI270_t *bmi270)
{
		...
		HAL_GPIO_WritePin(bmi270->cs_port, bmi270->cs_pin, GPIO_PIN_RESET);
		HAL_SPI_Transmit(bmi270->hspi, &reg_addr, 1, 100);

		//Send config file part to part
		while(byte_sent < CONFIG_SIZE)

		{
			uint16_t current_chunk = (CONFIG_SIZE - byte_sent > CHUNK_SIZE) ? CHUNK_SIZE : (CONFIG_SIZE-byte_sent);

			if(HAL_SPI_Transmit(bmi270->hspi, &bmi270_config_file[byte_sent], current_chunk, 500) != HAL_OK) {
			        Error_Handler();
			}

			byte_sent += current_chunk;
		}
		//Send config_file in one transmit
		/*	if(HAL_SPI_Transmit(hspi, (uint8_t*)bmi270_config_file, 8192, 5000) != HAL_OK) {
		    Error_Handler();
		}*/
		HAL_GPIO_WritePin(bmi270->cs_port, bmi270->cs_pin, GPIO_PIN_SET);
		HAL_Delay(2);
		...
		//Recive Status Code
		uint8_t chipID = read_CHIP_ID(bmi270);
		if(chipID == BMI270_CHIP_VALUE)
		{
			uint8_t tx[3] = {0x21 | 0x80,0x00,0x00};
			uint8_t rx[3] = {0,0,0};

			SPI_Recive(tx, rx, 3, 100,bmi270);
			HAL_Delay(1);
			return rx[2];
		}
