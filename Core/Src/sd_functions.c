...
FRESULT sd_open_log(const char *filename)
{
    ...
    return f_lseek(&logFile, f_size(&logFile));
}

FRESULT sd_write_log(const char *text,int *cont)
{
    UINT bw;

    FRESULT res = f_write(&logFile, text, strlen(text), &bw);
    ...
    if(*cont>=50){
    	res =f_sync(&logFile);   // fuerza a guardar en la SD
    	if(res!=FR_OK)
    		return res;
    	*cont=0;
    }
